...
def ES_search_path(fun, lbounds, ubounds, budget):
    ...
    while h==0 and budget>0 :
        ...
        p = np.concatenate((x_k, sel_u_best(u,x_k,fun)), axis = 0)   # add the selected parents to the original population to complete recombination and parent update
        p = sel_u_best(u, p,fun)                                     # select the u best solution to keep the population size constant
...
def happy(x,x_final,fun):
    ...
    """
    if abs(fun(x)-fun(x_final)) <= 0.001 :
        return 1
    else:
        return 0        
